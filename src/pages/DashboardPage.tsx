...
function GoalRow({ meta }: { meta: MetaWithResponsavel }) {
  ...
  return <Link to={`/metas/${meta.id}/analise`} className="grid gap-3 p-4 transition hover:bg-muted/30 md:grid-cols-[1fr_140px_120px] md:items-center"><div className="min-w-0"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full" style={{ background: STATUS_COLOR[status].fg }} /><span className="truncate text-sm font-semibold">{meta.nome}</span></div><div className="mt-1 pl-4 text-xs text-muted-foreground">{universoDaArea(meta.area)} · {formatValor(meta.valor_atual, meta.unidade)} de {formatValor(meta.valor_alvo, meta.unidade)}</div></div><div><div className="mb-1 flex justify-between text-[10px] text-muted-foreground"><span>Resultado</span><strong className="text-foreground">{progress}%</strong></div><ProgressBar value={progress / 100} status={status} height={5} /></div><div className="flex items-center justify-between text-xs"><span className="text-muted-foreground">Prazo</span><strong>{prazo}%</strong></div></Link>;
}
